...
func testWalletToOneAccountUsingAccount(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadAccount(ctx, tx, false, (*[]*Wallet)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testWalletToOneSetOpAccountUsingAccount(t *testing.T) {
	var err error
