...
func testPackageToManySubscriptions(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = a.L.LoadSubscriptions(ctx, tx, false, (*[]*Package)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testPackageToManyAddOpSubscriptions(t *testing.T) {
	var err error
