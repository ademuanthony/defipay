...
func MakeTemplateFuncMap() template.FuncMap {
	...
	return template.FuncMap{
		...
		"dateConversion": func(a int64) string {
			if a == 0 {
				return "N/A"
			}
			dateTime := time.Unix(a, 0).UTC()
			return dateTime.Format("2006-01-02")
		},
		"TimeConversion": func(a int64) string {
			if a == 0 {
				return "N/A"
			}
			dateTime := time.Unix(a, 0).UTC()
		...
		"formatBusd": func (amount int64) string {
			...
			baseAmount := amountBig.Quo(amountBig, big.NewFloat(1e4))
			amtFloat, _ := baseAmount.Float64()
			return fmt.Sprintf("%.4f", amtFloat)
		},
	}
}
